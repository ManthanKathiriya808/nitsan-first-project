...
console.log(content)
        ...
        replace : (domNode)=>{
            if(domNode.name == "h2"){
                return(
                    <h2 className=' text-[var(--secondryClr)] text-[42px]  leading-[54px]  mb-[10px] font-semibold '>
            ...
            if(domNode.name == "h6"){
                return(
                    <h6 className=' text-[20px]    my-[16px] leading-[28px]  '>
                        { domToReact(domNode.children,options)}
                    </h6>
                )
            }
            if(domNode.name == "a" && domNode.attribs?.class?.includes("btn__outline") ){
                return(
                    <a className='mb-[16px] '>
                        <Btn3 text={ domToReact(domNode.children,options)} />
                    </a>
                )
            }
            if(domNode.name == "a"  ){
               ...
               if(domNode.name == "span" && domNode.attribs?.class?.includes("gradient-color")){
                return(
                    ...
                    <h1 className='  bg-gradient-to-r  font-bold  from-[var(--primaryClr)] to-[var(--teritoryClr)] inline-block text-transparent bg-clip-text  '>
                        { domToReact(domNode.children,options)}
                    </h1>
                )
            }
  ...
  return (
    <div className='py-[64px]'>
        <div className="grid  grid-cols-1 lg:max-w-[1320px] py-[48px]  mx-auto  lg:grid-cols-2" >
