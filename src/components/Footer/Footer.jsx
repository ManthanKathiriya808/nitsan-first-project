...
const Footer = () => {
  ...
  return (
    <div className='mt-5 font-normal max-w-7xl mx-auto py-5  '>


      <div className="div text-start  mb-[80px] border-b border-[var(--textClr)]/10 " style={   { padding: "20px 0 10px"}}>
          <div className="class items-center  flex flex-col gap-6 lg:gap-0 lg:flex-row justify-between">
        <div className="links flex flex-col lg:flex-row  gap-[20px] text[16px]" style={{    padding: "20px 0 10px"}}>
          { data?.page?.footerNavigation.map((ele,index)=>(
            <div className="footerLinks" key={index}>
              <a href={ele.link} className="font-normal me-4 text-md">{ele.title}</a>
            </div>
          ))}
        </div>
      ...
      <div className="other gap-2 border-b border-[var(--textClr)]/10 font-normal  pb-[64px] grid grid-cols-1 lg:grid-cols-7">
          <div className="div col-span-2 footer-box">
          ...
          <div className="div col-span-3  footer-box">
             <h3 className='font-semibold text-[18px] mb-[14px] text-[var(--secondryClr)]'>
              {form?.header}
            </h3>
            <div className="box  w-[100%] ">
              <div className="upper relative">
                <input type="email" className='footer-form  border-gray-200 ' placeholder={form?.renderables[0]?.renderables[0]?.properties?.fluidAdditionalAttributes?.placeholder} required/>
                   <button className="relative  px-10  py-3 text-md font-medium text-white hover:text-[var(--primaryClr)] bg-[var(--primaryClr)] overflow-hidden footer-btn  group">
  ...
  <span className="absolute bottom-0 left-0 w-full h-0 bg-white border text-[var(--primaryClr)] border-[var(--primaryClr)] transition-all duration-500 ease-in-out group-hover:h-full"></span>
      ...
      <div className='text-center pt-5 font-'>
